...
const LOCATIONS = [
  'Jakarta', 'Bandung', 'Surabaya', 'Yogyakarta', 'Denpasar',
  'Medan', 'Makassar', 'Semarang', 'Palembang', 'Batam',
];
...
function buildOrganizations() {
  ...
  for (let i = 0; i < ORG_OWNER_COUNT; i++) {
    ...
    rows.push({
      ...
      contact_phone: `+62-8${String(1000000000 + i).slice(0, 10)}`,
...
function buildVolunteerProfiles() {
  ...
  for (let i = 0; i < VOLUNTEER_COUNT; i++) {
    ...
    rows.push({
      ...
      user_id: userId,
      phone_num: `+62-8${String(2000000000 + userId * 1000).slice(0, 10)}`,
